...
abs_tables <- generate_rarefied_abs_tables(physeq = trim_physeq, # Physeq with non-normalized counts (do not convert to rel abundance!)
...
rel_tables <- generate_rarefied_abs_tables(physeq = trim_physeq, # Physeq with non-normalized counts (do not convert to rel abundance!)
                              iterations = 100, # Number of rarefy steps
                              rare.depth = min(sample_sums(trim_physeq)), # Depth to rarefy to
...
time_res_df %>%
  ...
  annotate(geom = "text", y = 10, x = 0, label = "R^{2}==0.9982", parse = TRUE, hjust = 0)
```
