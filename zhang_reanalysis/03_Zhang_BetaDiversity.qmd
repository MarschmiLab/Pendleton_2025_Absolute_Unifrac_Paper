...
```{r timing, fig.height = 3, fig.width = 3}
...
rep_tests <- bind_rows(replicate(10, test_conditions, simplify=F))
...
load("time_results.RData")

time_res_df <- 
  time_results[1:96] %>%
  bind_rows() %>%
  cbind(rep_tests[1:96,]) %>%
  pivot_longer(Bray:Gunifrac, names_to= "Metric", values_to = "Time")

time_res_df %>%
  filter(Samples == 10, ASVs == 2000, Metric == "Gunifrac") %>%
  ggplot(aes(x = Alphas, y = Time/60)) + 
  geom_jitter() + 
  stat_summary(geom = "point", fun = median, size = 4, alpha = 0.6) + 
  stat_summary(geom = "errorbar",
               fun.min = \(x)median(x)-sd(x),
               fun.max = \(x)median(x)+sd(x),
               width = 0) + 
  labs(y = "Time (mins)", x = "Number of Alphas")

time_res_df %>%
  filter(Alphas == 1, ASVs == 2000, Metric == "Gunifrac") %>%
  ggplot(aes(x = Samples, y = Time/60)) + 
  geom_jitter() + 
  stat_summary(geom = "point", fun = median, size = 4, alpha = 0.6) + 
  stat_summary(geom = "errorbar",
               fun.min = \(x)median(x)-sd(x),
               fun.max = \(x)median(x)+sd(x),
               width = 0) + 
  labs(y = "Time (mins)", x = "Number of Samples")

time_res_df %>%
  filter(Alphas == 1, Samples == 10, Metric == "Gunifrac") %>%
  ggplot(aes(x = ASVs, y = Time/60)) + 
  geom_jitter() + 
  stat_summary(geom = "point", fun = median, size = 4, alpha = 0.6) + 
  stat_summary(geom = "errorbar",
               fun.min = \(x)median(x)-sd(x),
               fun.max = \(x)median(x)+sd(x),
               width = 0) + 
  labs(y = "Time (mins)", x = "Number of ASVs")
  
time_res_df %>%
  mutate(id = rep(c(1:6), each = 32)) %>%
  pivot_wider(names_from = Metric, values_from = Time) %>%
  mutate(Diff = Gunifrac - Bray,
         Diff_Perc = Gunifrac/Bray) %>%
  summarize(Median_Diff = median(Diff),
            Max_Diff = max(Diff),
            Median_DiffX = median(Diff_Perc),
            Max_DiffX = max(Diff_Perc),
            Median_Bray = median(Bray),
            Median_Gunifrac = median(Gunifrac))
