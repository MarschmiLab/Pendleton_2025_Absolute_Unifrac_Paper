...
These don't have primers in their start (so forward reads don't have forward primers at the beginning) BUT they do over-extend past the V4 region, meaning that you do get reverse primers in the forward reads, and vice versa. Will be important for trimming later on.
...
sample_names_raw <- 
  basename(forward_reads_raw) %>%
  str_remove("_1.fastq.gz")

# Choose only samples we want to work with

keeps <- sample_names_raw %in% meta$Run

forward_reads <- forward_reads_raw[keeps]
reverse_reads <- reverse_reads_raw[keeps]
sample_names <- sample_names_raw[keeps]
...
random_plots <- sample(1:93, size = 6)
...
Reverse reads get pretty rough, right around 150. I going to truncate there. For forward, I'm okay using as much of the read as possible (truncLen 253).
...
```{r size selection}
asv_keeps <- nchar(getSequences(seqtab_nochim)) ==253
...
# Removed two ASVs that were mitochondria
save(clean_barlow_phylo, file =  "clean_barlow_phylo.RData")
```
