...
rarefaction_limit <- min(sample_sums(full_diversity_physeq))

depths <- c(1, 250, 500, 1000, 5000, 10000, rarefaction_limit)

diff_depth_tables <- 
  map(depths, \(x){
  generate_rarefied_abs_tables(physeq = full_diversity_physeq,
                              iterations = 10, 
                              rare.depth = x, 
...
#   })
# 
# save(diff_depth_dists, file = "diff_depth_dists.RData")

load("diff_depth_dists.RData")
...
mantel_results %>%
  ...
  scale_color_manual(values = volcano) + 
